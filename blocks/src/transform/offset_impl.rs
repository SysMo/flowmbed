...
use super::offset_auto::*;

/// Implementation DynamicalSystem protocol
#[allow(unused_variables)]
impl<'a> ds_core::DynamicalSystem<'a> for Offset<'a> {
  fn init(&mut self) -> anyhow::Result<()> {
    // >>> Begin section @DynamicalSystem::init
    self.output.initialize(*self.input + *self.offset);
    Ok(())
  ...
  fn step(&mut self, ssi: &ds_core::SystemStateInfo) -> anyhow::Result<()> {
    ...
    self.output.update(*self.input + *self.offset, ssi);
    Ok(())
